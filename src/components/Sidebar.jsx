...
import {SiShopware, SiDatabricks} from 'react-icons/si';
...
const Sidebar = () => {
  ...
  // function for closing sidebar on mobilescreen , on click of any menu
      ...
      {activeMenu && (<>
        <div className="flex justify-between items-center">
          ...
          font-extrabold tracking-tight dark:text-white text-slate-900'>
            <SiDatabricks/> <span>StatBoard</span>
          </Link>
        ...
        <div className="mt-10">
          {/* First, taking the title of the links   */}
          {links.map((item) => (
            <div className=""
            key={item.title}>
                <p className='text-gray-400 m-3 mt-4 uppercase'>{item.title}</p>
                
                {/* Taking the name of the links using  */}
                {item.links.map((link) => (
                  <NavLink
                  to={`/${link.name}`}
                  key={link.name}
